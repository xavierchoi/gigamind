...
| MRR | 0.610 | +0.039 (+6.8%) |
| Recall@10 | 81% | -3%p |
| Latency P95 | **6,433ms** | **+25.1x** |
| Latency P50 | 4,979ms | +22.7x |

### Phase 6 A/B 테스트 결과 (v0.6.0)

**개선점:**
- Hit@1: 39% → 47% (+8%p, 목표 55% 미달성)
- MRR: 0.571 → 0.610 (+6.8%)

**문제점:**
- Latency 급증: P95 256ms → 6,433ms (목표 800ms 대비 8배 초과)
- Recall@10 감소: 84% → 81% (-3%p)

**결론:**
- LLM Reranking은 정확도를 개선하지만 현재 구현은 **프로덕션에 비적합**
- **기본값 OFF 유지 권장**
- 개선 방안: 배치 요청, 캐싱, 선택적 리랭킹

### Phase 6 구현 상세 (v0.6.0)
