...
**현재 실패 패턴**:
```
"SF에서 자율주행차 탔어?" → 테슬라 로보택시 노트 못 찾음
"미국 마트에서 뭘 샀어?" → Trader Joe's 노트 못 찾음
```
...
**A. 동의어/관련어 확장**
```typescript
// 쿼리 확장 맵 (수동 또는 LLM 생성)
const expansionMap = {
  "자율주행차": ["로보택시", "테슬라", "웨이모", "self-driving"],
  "미국 마트": ["Trader Joe's", "홀푸드", "grocery"],
  // ...
};

function expandQuery(query: string): string[] {
  const terms = extractKeyTerms(query);
  const expanded = terms.flatMap(t => expansionMap[t] || [t]);
  return [query, ...generateVariants(query, expanded)];
}
```
...
- `src/rag/retriever.ts` (확장된 쿼리 사용)
...
2. **Phase 4.1**: 쿼리 확장 → Hit@1 ↑
3. **Phase 2.1**: 중형 vault (500개) → 확장성 검증
4. **(선택)**: GPU 서버 → 참조: `docs/embedding_strategy.md`
