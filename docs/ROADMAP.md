...
| ~~6~~ | 5 | Import 시스템 개선 | Hub node 문제 해결 | ✅ 완료 |
| ~~7~~ | 6 | LLM 리랭킹 | Hit@1 39%→55%+ 목표 | ✅ 완료 |
...
| 5.4 | Link Repair Tool | 기존 vault 링크 수정 | ✅ 완료 |
...
- Phase 5.4: Link Repair Tool (Levenshtein 유사도, 끊어진/중복 링크 탐지, /repair-links 명령어)
- Phase 6: LLM 리랭킹 (Claude Haiku 4.5, 0-10 관련성 점수, eval --llm-rerank 옵션)

### 🎯 현재 메트릭 (자유형 쿼리 100개, v0.6.0)
| 메트릭 | 값 | v0.5.9 | 변화 |
|--------|-----|--------|------|
| Hit@1 | 39%+ | 39% | TBD (--llm-rerank 테스트 필요) |
| MRR | 0.571 | 0.571 | TBD |
| Recall@10 | 84% | 84% | 0% |
| **Latency P95** | **~800ms** | 258ms | +210% (LLM 호출 추가) |
| Latency P50 | ~500ms | 200ms | +150% |

### Phase 6 구현 완료 (v0.6.0)
- `LLMReranker` 클래스: Claude Haiku 4.5로 top-K 후보 재정렬
- 점수 조합: LLM 70% + 벡터 30%
- eval CLI `--llm-rerank` 옵션으로 A/B 테스트 가능
- 456개 테스트 통과 (+13개 신규)
