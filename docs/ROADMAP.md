...
| **1** | - | 레이턴시 최적화 | P95 < 500ms 목표 | ⏳ 대기 |
| **2** | 4.2 | 그래프 리랭킹 | Hit@1 추가 개선 | ⏳ 대기 |
| **3** | - | GPU 서버 (선택) | 파워유저용 | ⏳ 대기 |
...
### 🎯 현재 메트릭 (자유형 쿼리 100개, v0.5.2)
| 메트릭 | 값 | 이전 | 변화 |
|--------|-----|------|------|
| Hit@1 | 40% | 39% | +2.6% |
| MRR | 0.5775 | 0.5740 | +0.6% |
| Recall@10 | 84.5% | 81.5% | +3.7% |
| Latency P95 | 296ms | 980ms | -70% |

### ⏳ 다음 작업 (순서대로)
1. **Phase 2.1**: 중형 vault (500개) → 확장성 검증
2. **(선택)**: GPU 서버 → 참조: `docs/embedding_strategy.md`
